...
def transform_video(video, ref_images, ref_audio, target_lang, prompt, style, skip_lipsync, preserve_bg, smooth, use_mask, use_lcm, show_comparison, progress=gr.Progress()):
    ...
    logger.info(f"UI transformation. Style: {style}, LCM: {use_lcm}, Smooth: {smooth}, Mask: {use_mask}")
...
with gr.Blocks(title="Video & Audio Transformer", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Tab("Transform"):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    input_ref_images = gr.File(label="2. Reference Character Images", file_count="multiple", file_types=["image"])
                    input_ref_audio = gr.Audio(label="3. Reference Voice Audio", type="filepath")
                    with gr.Group():
    ...
    run_btn.click(fn=transform_video, inputs=[input_video, input_ref_images, input_ref_audio, target_lang, prompt, style, skip_lipsync, preserve_bg, smooth, use_mask, use_lcm, show_comparison], outputs=[output_video, comparison_video, status, history_display])
